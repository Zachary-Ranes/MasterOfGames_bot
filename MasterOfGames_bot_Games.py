...
class Game(object):
    ...
    def __init__(self, game_code = None, min_players = None, max_players = None):
        ...
        #holds the last messages to each player used to edit all the player messages at once
        self.last_messages = {}

    #Check the game type
    def game_type(self, game):
        if game == "resistance":
            return self.game_code == 1
        if game == "mafia":
            return self.game_code == 2
        else:
            return False
    ...
    def add_player(self, player_id, player_username, player_name):
        if self.game_state != 0 or player_id in self.ids_of_players:
            return False
    ...
    def can_start_game(self, player_id):
        if player_id == "false_start":
            game_state = 0
            return
        if self.game_state != 0 or player_id not in self.ids_of_players:
            return False
        ...
        if self.number_of_players < self.MIN_PLAYERS:
            self.message_for_group[0] = ("Not enough players have joined to start the game.\n"\
                                         "We need "+str(self.MIN_PLAYERS)+" to "\
...
class Resistance(Game):
    ...
    def setup_round(self):
        ...
        self.message_for_group[0] = ("@"+ self.players_id_to_username[self.id_of_nominator] 
                                 ...
                                 +" players to go on the mission. "+ extra_message
                                 +"\nNominate players by typing /nominate (space) @username "
                                 +"(space) @username ...etc")
        self.message_for_group[1] = None
        self.message_for_players = {}
    
    #Function returns true and changes group message if the game has gotten to a wining state
    def win_state(self):
    ...
    def nomination_vote_logic(self, player_id, vote):
        ...
        if vote == "yea":
            ...
            if self.nominees_yea_votes > self.number_of_players/2:
                self.game_state = 4
                self.message_for_group[0]+=("\nEnough yea votes have been cast, the mission will "\
                                            "take place.")
                self.message_for_group[1]=None
                return "Pass"
        if vote == "nay":
            ...
            if self.nominees_nay_votes >= self.number_of_players/2:
                self.game_state = 2
                self.message_for_group[0]+=("\nEnough nay votes have been cast, the next player "\
                                            "will get to nominate someone now.")
                self.message_for_group[1]=None
                return "Fail"
        return True
