...
class Mafia(Game):
    ...
    def setup_game(self):
        ...
        self.number_of_innocents = self.number_of_players - self.number_of_alive_mafiosi
        ...
        for player_id in self.ids_of_mafiosi:
            if player_id in self.ids_of_alive_players:
                self.message_for_players[player_id] = ("Who do you want to kill? Once all the "\
                                                       ...
                                                       markup_mafia)
        if id_of_doctor in ids_of_alive_players:
            self.message_for_players[id_of_doctor] = ("Who do you want to heal tonight?", 
                                                      markup_doctor)
        if id_of_detective in ids_of_alive_players:
            self.message_for_players[id_of_detective] = ("Who do you want to check the role of?", 
                                                         markup_detective)
        self.ids_of_mafia_targets = {}
    ...
    def kill_vote_logic(self, mafia_id, id_of_player_voted_to_be_killed):
        ...
        for i in range(len(self.ids_of_innocents)):
            ...
            if player_id in self.ids_of_alive_players:
                ...
                vote_count = sum(1 for targets_id in self.ids_of_mafia_targets.values() \
                                 if targets_id==id_of_player_voted_to_be_killed)

                markup_mafia.row(types.InlineKeyboardButton(
                    callback_data="kill"+ player_id_index +str(key), 
                    text="@"+self.players_id_to_username[player_id]+vote_count))
    ...
    def setup_day(self):
        ...
        if victim_kill:
            ...
            self.number_of_innocents = self.number_of_innocents - 1
        else:
            self.message_for_group[0] = ("During the night @" 
                                        +self.players_id_to_username[id_of_victim] 
        ...
        for player_id in self.ids_of_players:
            if player_id in slef.ids_of_alive_players:
    ...
    def lych_logic(self, voter_id, id_of_player_to_be_lyched):
        ...
        vote_count = sum(1 for lych_id in self.ids_of_lych_targets.values() \
                         if lych_id==id_of_player_to_be_lyched)

        markup_lych = types.InlineKeyboardMarkup()
        for player_id in self.ids_of_players:
            markup_lych.row(types.InlineKeyboardButton(
                callback_data="lynch"+str(player_id), 
                text="@"+self.players_id_to_username[player_id]+" "+vote_count))
        self.message_for_group[1] = markup_day

        if vote_count >= (len(self.ids_of_alive_players)/2):
            self.ids_of_alive_players.remove(id_of_player_to_be_lyched)
            if id_of_player_to_be_lyched in self.ids_of_mafiosi:
    ...
    #Method will change group message and game_state if a victory state is reached
    def check_for_winner(self):
        if self.number_of_innocentsr == 0:
